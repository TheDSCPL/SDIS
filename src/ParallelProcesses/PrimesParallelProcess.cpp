...
PrimesSeriesProcess::PrimesSeriesProcess(Connection* c, unsigned long long int maxN) : ParallelizableProcess(c), maxN(maxN) {
    flags.push_back(false);
    for(int i = 1; i<maxN ; i++)
        flags.push_back(true);
...
PrimesSeriesProcess::~PrimesSeriesProcess() {
    for(SimpleTask* t : primeCalculators)
        delete t;
}

void PrimesSeriesProcess::onReady() {
    for(int i = 0 ; i<maxN ; i++) {
        if(flags[i]) {
            if (conn)
                (*conn) << i + 1 << " ";
